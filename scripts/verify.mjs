...
function verifyContrast(css) {
  for (const mode of ['light', 'dark']) {
    ...
    for (const [label, foreground, background, minimum] of [
      ['text', '--gala-color-text', '--gala-color-background', 4.5], ['muted text', '--gala-color-muted', '--gala-color-background', 4.5],
      ['link', '--gala-color-accent', '--gala-color-background', 4.5], ['surface text', '--gala-color-text', '--gala-color-surface', 4.5],
      ['surface muted text', '--gala-color-muted', '--gala-color-surface', 4.5], ['surface link', '--gala-color-accent', '--gala-color-surface', 4.5],
...
function declarations(css, selector) {
  ...
  for (const declaration of match[1].matchAll(/(--[a-z0-9-]+)\s*:\s*([^;]+);/gi)) values.set(declaration[1], declaration[2].trim());
  return values;
}
function hex(value) { const colors = String(value ?? '').match(/#[0-9a-f]{6}\b/gi); requireValue(colors?.length, `expected a six-digit color in ${value}`); return colors.at(-1); }
...
function luminance(value) { const c = value.slice(1).match(/../g).map((p) => Number.parseInt(p, 16) / 255).map((x) => x <= 0.04045 ? x / 12.92 : ((x + 0.055) / 1.055) ** 2.4); return 0.2126 * c[0] + 0.7152 * c[1] + 0.0722 * c[2]; }
function safeCssPath(value) { return typeof value === 'string' && value.length <= 200 && value.endsWith('.css') && !value.startsWith('/') && !value.includes('\\') && value.split('/').every((part) => /^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(part)); }
function compareVersions(a, b) { const left = a.split('.').map(Number); const right = b.split('.').map(Number); for (let i = 0; i < 3; i += 1) if (left[i] !== right[i]) return left[i] - right[i]; return 0; }
function exactFields(value, expected, label) { requireValue(value && typeof value === 'object' && !Array.isArray(value), `${label} must be an object`); requireValue(JSON.stringify(Object.keys(value).sort()) === JSON.stringify([...expected].sort()), `${label} fields are invalid`); }
