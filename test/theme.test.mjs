...
test('ships the immutable Flash release contract', async () => {
  const { manifest, cssBytes } = await verifyTheme();
  const packageJson = JSON.parse(await readFile(path.join(ROOT, 'package.json'), 'utf8'));

  assert.equal(manifest.id, 'flash');
  ...
  assert.equal(packageJson.name, '@rathnasgala/theme-flash');
...
async function withFixture(assertions) {
  const temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), 'gala-flash-'));
  const fixture = path.join(temporaryDirectory, 'theme-flash');
  try {
    await mkdir(fixture);
